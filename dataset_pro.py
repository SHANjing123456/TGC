...
def get_adjacent_matrix(distance_file: str, num_nodes: int, id_file: str = None, graph_type="connect") -> np.array:
    ...
    with open(distance_file, "r") as f_d:
        ...
        for item in reader:  # 将一行给item组成列表
            ...
            if graph_type == "connect":  # 这个就是将两个节点的权重都设为1，也就相当于不要权重
                A[i, j], A[j, i] = 1., 1.
            elif graph_type == "distance1":  # 这个就是将两个节点的权重都设为1，也就相当于不要权重
                A[i, j] = 1. / distance
                A[j, i] = 1. / distance
            elif graph_type == "distance2":  # 这个是有权重，下面是权重计算方法
                if distance <=4.69:
                    A[i, j] = math.exp(-distance ** 2 / 3.54)#3.19116098615917  标准差为: 1.7863820941106552
                    A[j, i] = math.exp(-distance ** 2 / 3.54)
                else:
                    A[i, j] = 0
                    A[j, i] = 0
            else:
            ...
            #把A保存为npy文件
            np.save(f'A_{graph_type}.npy',A)

    return A
def hybrid_decay(distance, threshold=6):
    if distance < threshold:
        return 1 - distance / threshold  # 线性衰减
    else:
        return math.exp(-distance ** 2 / 16)  # 指数衰减
def log_decay(distance, base=10):
    return np.log(distance + 1) / np.log(base)
def power_decay(distance, exponent=2):
    return 1 / (distance ** exponent)
def sine_decay(distance, period=10):
    return np.sin(np.pi * distance / period)
def exp_linear_decay(distance, exp_coeff=0.5, linear_coeff=1):
    return exp_coeff * np.exp(-distance) + linear_coeff * (1 - distance / 100)
def custom_decay(distance):
    if distance < 10:
        return 1 - distance / 10
    else:
        return math.exp(-distance ** 2 / 16)
...
def get_flow_data(flow_file: str) -> np.array:  # 这个是载入流量数据,返回numpy的多维数组
    ...
    data1 = data["积水"].reshape(1488, 13, 1)
    data2 = data["降水"].reshape(1488, 13, 1)
    data = np.concatenate((data1, data2), axis=2)
    flow_data = data.transpose([1, 0, 2])  # [N, T, D],transpose就是转置，让节点纬度在第0位，N为节点数，T为时间，D为节点特征
    # [:, :, 0]就是只取第一个特征，[:, :, np.newaxis]就是增加一个维度，因为：一般特征比一个多，即使是一个，保持这样的习惯，便于通用的处理问题
    # 原数据(16992, 307, 3) 转成 (307,16992,3) 307为节点数，16992为时间，3为节点特征(交通流量，平均速度，平均占用率)
...
class LoadData(Dataset):  # 这个就是把读入的数据处理成模型需要的训练数据和测试数据，一个一个样本能读取出来
    def __init__(self, data_path, num_nodes, divide_days, history_length, train_mode,graph_type):
        """
        ...
        self.train_days = divide_days[0]  # 59-14 = 45, train_data  total550days  440
        self.test_days = divide_days[1]  # 7*2 = 14 ,test_data   110
        self.history_length = history_length  # 历史长度为6
        self.one_day_length = 24  # 一整天的数据量
        self.graph_type = graph_type
        self.graph = get_adjacent_matrix(distance_file=data_path[0], num_nodes=num_nodes,graph_type=graph_type)
    ...
    def __getitem__(self, index):  # 功能是如何取每一个样本 (x, y), index = [0, L1 - 1]这个是根据数据集的长度确定的
        ...
        # print(data_x[:,:,0].unsqueeze(2),'-')
        # print(data_x[:,:,1].unsqueeze(2),'*')
        # print("data_x维度取第一维",data_x[:,:,0].unsqueeze(2).shape)
        # print("data_x维度",data_x.shape)
        data_y = LoadData.to_tensor(data_y).unsqueeze(1)  # [N, 1, D]　# 转换成张量，在时间维度上扩维
        # print("data_y维度", data_y.shape)
        # print("data_y维度取第一维",data_y[:,:,0].unsqueeze(-1).shape)

        return {"graph": LoadData.to_tensor(self.graph), "flow_x": data_x, "flow_y": data_y[:,:,0].unsqueeze(-1)}  # 组成词典返回
...
if __name__ == '__main__':
    train_data = LoadData(data_path=["cites.csv", "hy_rain.npz"], num_nodes=15,
                          divide_days=[49, 13],history_length=6,train_mode="train",graph_type="distance2")
